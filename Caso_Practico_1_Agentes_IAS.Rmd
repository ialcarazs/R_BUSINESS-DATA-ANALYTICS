...
#Librerías
paquetes <- c('tidyverse','lubridate','plotly','DataExplorer')
...
read_lines('./CasosPracticos/Caso1/AnalisisAgentes.csv', n_max = 3)
...
df <- read_csv('./CasosPracticos/Caso1/AnalisisAgentes.csv')
...
df %>% 
  mutate(any = year(f_desde)) %>% 
  count(any) %>% 
  ggplot(aes(any,n)) + geom_col()
...
df <- df %>% 
  mutate(antiguedad_meses = trunc(time_length(f_hasta - f_desde, unit= 'month')),
         antiguedad_any = trunc(time_length(f_hasta - f_desde, unit = 'year'))
         )
...
ggplot(df, aes(antiguedad_any)) + geom_bar()
...
df %>% 
  ...
  mutate(ranking_prod = row_number(desc(produccion)) , 
         porcentaje = cumsum(produccion) / sum(produccion)) %>% 
  ggplot(aes(x=ranking_prod, y=porcentaje)) + geom_line()
...
df %>% 
  ...
  mutate(ranking_ctr = row_number(desc(cartera)),
         por_ctr = cumsum(cartera) / sum(cartera) ) %>% 
  ggplot(aes(x=ranking_ctr, y= por_ctr)) + geom_line()
...
df <- df %>% 
  mutate(ranking_prod = row_number(desc(produccion)),
         ranking_ctr = row_number(desc(cartera))) 
...
cor(df$antiguedad_any,df$ranking_prod)

cor(df$antiguedad_any,df$ranking_ctr)
...
df %>% 
  group_by(antiguedad_any) %>% 
  summarise(media_prod_any = mean(produccion),
         media_ctr_any = mean(cartera)) %>% 
  print(n = Inf)
...
df %>% 
  ...
  summarise(media_prod_any = mean(produccion),
         media_ctr_any = mean(cartera)) %>% 
  ggplot(aes(x= antiguedad_any))  +
  geom_col(aes(y= media_prod_any*10 ), alpha= 0.3, fill = 'red') + 
  geom_col(aes(y= media_ctr_any ), alpha= 0.3, fill = 'green')
...
Pero vamos a profundizar más, vamos a crear una matriz estratégica y posicionar a cada agente

```{r}

ggplotly(
ggplot(df,aes(cartera,produccion)) + geom_point() + geom_smooth() )
...
cuotas_zonas <- filter(contribucion, dimension == 'zona') %>% 
  select(-cartera)
```
